...
### v0.3.2 — RATIFIED · quality-and-disposition only; enforcement in packet + Box 8

> **RATIFIED · 2026-06-22.** *Revs v0.3.1.* The production line-generation prompt. Carries **voice, craft, disposition, block-execution**; the **packet** carries facts and constraints; **Box 8** is final permission to air. Per TL's correction: the prompt is quality-critical, safety-insufficient.
>
> *v0.3.2 — fixes the one real contradiction flagged in review: the prompt previously handed the model the **raw post** "for register only," which is an honor system (every forbidden detail is visible, restraint merely requested) and contradicts "allowed claims are the only facts." Replaced with **REGISTER HINT** (the tone to match) + **PERMITTED SOURCE SPANS** (exact wording it may echo). The raw post stays in the audit record and upstream meaning pass — it is no longer exposed to line generation. This is genuine input-constraint (Pillar 2), not a prompt-level request.*
>
> *v0.3.1 — Pillar 6 audit fix + the PLAINLY-STATED-SERIOUS-FACT ⊆ ALLOWED-CLAIMS invariant. v0.3.0 — fixed three contradictions; added BLOCK CONTRACT and PLAINLY STATED SERIOUS FACT fields.*
>
> **Status: ratified. Next: the 50-post run with gold packets + packet-preflight — no more prompt drafting.**
...
REGISTER HINT:        [playful / weary / proud / plain / restrained]
PERMITTED SOURCE SPANS: [exact wording the line may echo, or none — raw post is NOT supplied; it stays in the audit record only]
...
**Pillar 2 — constrain the input.** *Passes — and now genuinely.* The raw "here's a post, say your bit" wrapper is gone; the structured packet replaces it. ALLOWED CLAIMS is the load-bearing field. **The v0.3.2 fix closed the last honor-system hole:** the model is no longer handed the raw post "for register only" (which exposed every forbidden detail and merely asked for restraint) — it gets a REGISTER HINT and PERMITTED SOURCE SPANS instead, so it can't lift an unvetted detail because *it was never given one.* That's real input-constraint, not a request. **Dependency, not a flaw:** this only works if the packet is populated — against empty claims the redirect moves become the only defense. So the prompt requires populated (gold) packets to be tested honestly, and a packet-preflight gate to refuse malformed ones.
